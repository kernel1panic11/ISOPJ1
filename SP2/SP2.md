...
<main class="contenedor-principal">
    ...
    <div class="content-section">
        ...
        <ul>
            ...
            <li>Tipos de formateo
                <ul>
                    <li>Bajo nivel: lo que hace es borrar todo; sobrescribe todo con 000000 e intenta reparar los sectores defectuosos, pero se necesitan programas específicos; no se puede hacer a través del sistema operativo.</li>
                    <li>Medio nivel: sería un intermedio, no borra archivos; los sectores defectuosos que encuentra los marca, pero no intenta arreglarlos. También se pueden recuperar los archivos.</li>
                    <li>Alto nivel: no borra los archivos, solo borra el sistema de archivos e ignora los sectores defectuosos que pueda encontrar.</li>
                </ul>
            </li>
        </ul>
    </div>
    ...
    <div>
        <p>Para empezar, crearemos un disco virtual nuevo en VMware. Lo normal es crear un disco SATA, pero en mi caso he indicado NVMe ya que así va más <strong>rápido</strong>. Como se puede ver, el disco duro es de 10 GB. </p>
        ...
        <p>Ahora, con <code>df -Th</code>, podemos ver el sistema de archivos que se usa, el espacio que <strong>está</strong> usado y el libre que hay. El parámetro <code>-h</code> es opcional, pero lo pone “para humanos”; entonces lo representa en GB y así es <strong>más</strong> <strong>fácil</strong> de interpretar.</p>
        ...
        <p>La medida de clúster (Windows) y de bloque (Linux) es la unidad mínima lógica donde se guardan los datos a nivel de sistema operativo. Por defecto son 4096 bytes (es igual a 8 sectores) y esta medida <strong>sí</strong> se puede cambiar. Esta medida se cambia cuando se formatea la partición, y cada partición del disco <strong>puede</strong> tener una medida de bloque y un sistema de archivos <strong>diferentes.</strong> Este es un claro ejemplo: podemos ver <strong>cómo</strong> el archivo pesa 17 bytes y luego su tamaño real es de 5 KB.</p>
        ...
        <p><strong>Particiones</strong>: una <strong>partición</strong> es un trozo <strong>físico</strong> del disco duro, pero no podemos modificar la medida del bloque directamente; eso se hace por terminal. Un volumen es una capa de <strong>abstracción</strong> que se pone encima de las particiones y/o discos. </p>
        <p>GParted sirve para gestionar <strong>particiones</strong>.</p>
        ...
        <p>Aquí hemos creado dos particiones de <strong>más</strong> o menos 5 GB cada una. Esto lo hemos hecho usando la terminal. Como se puede ver en la imagen, las dos particiones se han creado correctamente.</p>
        ...
        <p>Aquí lo que he hecho es darle formato a la primera <strong>partición</strong> en ext4, usando el comando <code>mkfs.ext4 -b 2048 /dev/nvme0n1p1</code>.</p>
        ...
        <p>Ahora, aquí lo que he hecho es darle formato a la <strong>partición</strong> secundaria en <strong>NTFS</strong>, usando el comando <code>mkfs.ntfs /dev/nvme0n1p1</code>. En este caso ha tardado <strong>más</strong> que con ext4 ya que tiene que llenar la <strong>partición</strong> de <strong>ceros</strong>. Desconozco el <strong>por qué</strong>.</p>
    ...
    <div>
        ...
        <p>Para empezar, montaremos de forma temporal una <strong>partición</strong> en <code>/mnt/p1</code>. <strong>Podríamos</strong> montarla en cualquier otra carpeta, pero lo haremos aquí. Para montar la <strong>partición</strong> en este caso, utilizaremos el comando <code>mount -t ext4 /dev/nvme0n1p1 /mnt/p1</code> y, dentro de la <strong>partición</strong> <strong>recién</strong> montada, crearé una carpeta llamada “test” para luego reiniciar la VM y comprobar si sigue montada la <strong>partición</strong> y aparece la carpeta, lo cual ya adelanto que no es el caso. </p>
        ...
        <p>Montaje de <strong>partición</strong> en modo permanente:</p>
        ...
        <p><strong>Comprobación</strong> <strong>después</strong> del reinicio:</p>
    ...
    <div class="content-section">
        ...
        <p>En <code>/etc/shadow</code> se encuentran las contraseñas de los usuarios cifradas. Se puede manipular el algoritmo usado en el sistema editando otro archivo. También se ocupa de controlar la caducidad de las contraseñas.</p>
        ...
        <p>Aquí lo que hago es crear un usuario con <code>adduser</code>. Con este comando automáticamente se creará la carpeta <code>/home</code>, pero no los archivos dentro de la home; esos se crearán cuando inicies sesión con él.</p>
        ...
        <p>Cuando creas el usuario, no se crean las carpetas de la home hasta que inicies sesión con ese usuario.</p>
        ...
        <p>Al borrar un usuario, no se borra su home.</p>
        ...
        <p>Este comando sirve para establecer el grupo principal de un usuario. Un usuario puede estar en un <strong>único</strong> grupo principal, pero en todos los grupos secundarios que quiera.</p>
        ...
        <p><strong>También</strong> he editado para que la home del usuario sea <code>/k/</code>.</p>
        ...
        <p>Aquí ponemos cada <strong>cuánto</strong> tiempo tiene que expirarle la contraseña.</p>
        ...
        <p>Como se puede ver, da otro error. Esto es porque lo tenemos en <code>/var</code>.</p>
